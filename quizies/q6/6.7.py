...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        if not myrobot.check_collision(grid):
            return [False, 1, myrobot.num_steps]
            print '##### Collision ####'
...
###################################
found, col, steps = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
       p[0], p[1], p[2], p[3])
best = steps
...
while T > 0.00000001:
    ...
    found, col, steps = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
           p[0], p[1], p[2], p[3])
    delta = steps - best
    if found and not col and (steps < best or random.random() < e ** (-delta / T)):
        if steps < best:
            last = 0
            abs_p = p[:]
            abs_best = steps
